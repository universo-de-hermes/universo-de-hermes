    ...
    <div class="skill-item-v2 ${isExpanded ? 'expanded' : ''}" data-slug="${skill.slug}">
      ...
      ${isExpanded ? `
        <div class="skill-details">
          <div class="skill-desc">${escapeHtml(skill.description || 'Sin descripción')}</div>
...
function render() {
  ...
  container.innerHTML = `
    ...
    <div class="skills-list-v2" id="skills-list">
      ${filtered.length === 0
        ? `<div class="skills-empty">SIN HABILIDADES</div>`
        : filtered.map(renderSkillItem).join('')
      }
    </div>

    <div class="skills-stats">
      <span>${counts.workspace} LOCALES</span>
      <span class="skills-stats-sep">|</span>
      <span>${counts.global} GLOBALES</span>
      <span class="skills-stats-sep">|</span>
