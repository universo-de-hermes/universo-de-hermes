...
export function initControls() {
  ...
  document.getElementById('reset-btn')?.addEventListener('click', function () {
    ...
    showNotification('TODO RESTABLECIDO');
  });

  document.getElementById('save-btn')?.addEventListener('click', function () {
    savePanelPositions();
    showNotification('DISTRIBUCIÓN GUARDADA');
  });
...
function initThemePalette() {
  ...
  THEME_PRESETS.forEach(preset => {
    ...
    btn.addEventListener('click', function() {
      ...
      setThemeHue(hue);
      showNotification(`TEMA CAMBIADO A ${preset.name.toUpperCase()}`);
  ...
  try {
    ...
    data.engines.forEach(engine => {
      ...
      btn.addEventListener('click', async () => {
        ...
        if (r.ok) {
          showNotification(`MOTOR TTS: ${engine.name.toUpperCase()}`);
...
async function refreshProfile() {
  ...
  try {
    ...
    if (profile.telegram_chat_id) parts.push(`💬 ID: ${profile.telegram_chat_id}`);
    
    if (parts.length) {
      infoEl.innerHTML = parts.join('<br>');
      infoEl.style.color = 'var(--accent-primary, #00d4ff)';
      if (indicator) indicator.textContent = `${parts.length} dato(s)`;
    } else {
      infoEl.textContent = 'Sin datos guardados';
      infoEl.style.color = 'var(--text-secondary, #888)';
    ...
    clearBtn.addEventListener('click', async () => {
      ...
      try {
        const res = await fetch('/api/profile/delete', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ keys: ['name', 'telegram_chat_id'] }),
        });
        const data = await res.json();
        if (data.ok) {
          refreshProfile();
          showNotification('DATOS BORRADOS');
        }
      } catch {}
    });
  }
...
// Llamar desde initControls
const origInit = initControls;
const profileInit = initProfileManager;
