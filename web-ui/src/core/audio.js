...
function playTTSAudio(url, label, onEnd) {
  try {
    if (!isAudioInitialized && !initAudio()) return;
    ensureAudioContextStarted();
    const audioPlayer = createNewAudioElement();
    ...
    // También llamar onEnd cuando termine (el ended event del createNewAudioElement puede no funcionar con TTS)
...
export function initAudioControls() {
  ...
  window.addEventListener('tts-play', async (e) => {
    const { url, label, onEnd } = e.detail;
    if (!isAudioInitialized) initAudio();
    // 等待 resume 完成再播放
    if (audioContext && audioContext.state === 'suspended') {
      try { await audioContext.resume(); } catch {}
    }
    playTTSAudio(url, label, onEnd);
  });
