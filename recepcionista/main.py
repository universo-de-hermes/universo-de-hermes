...
# OJO: esto NO puede calcularse una sola vez al arrancar. Pepe queda corriendo
# días enteros; si las fechas se congelan, «mañana» apunta al día siguiente del
# arranque y las citas salen mal sin que nadie se entere.
DIAS_ES = ["lunes", "martes", "miércoles", "jueves", "viernes", "sábado", "domingo"]


def bloque_fechas() -> str:
    """El mapa de fechas de HOY, recalculado en cada mensaje."""
    hoy = datetime.now()
    prox = [(hoy + timedelta(days=i)) for i in range(14)]
...
def prompt_de_hoy() -> str:
    """El prompt con las fechas del día en que se contesta, y las reglas de
    agendamiento con herramientas."""
    return (SYSTEM_PROMPT
            .replace("__FECHAS_DE_HOY__", bloque_fechas())
            .replace("__COMO_AGENDA__", BLOQUE_PROMPT))


# se dejan por compatibilidad con lo que ya los importaba
FECHA_HOY = datetime.now().strftime("%d/%m/%Y")
DIA_SEMANA = DIAS_ES[datetime.now().weekday()]
FECHAS_AYUDA = bloque_fechas()
...
Anticipación mínima: 30 minutos. La disponibilidad real la consulta Pepe en la agenda.
...
- Bogotá: Diana Carolina Ruiz (Cosmetóloga) | Medellín: Valentina Baquero (Cosmetóloga)
...
- ✅ Medellín: Valentina Baquero (Cosmetóloga) | 1h 30min | $190.000 COP
...
El cliente puede tener varias citas agendadas al mismo tiempo (ej: Carbon Peel y Depilación Láser). No hay restricción. Cada nueva solicitud de cita sigue el flujo normal: ciudad → sede → servicio → fecha → hora → datos → resumen → confirmación, y la agendás tú.
...
async def ask_pepe(user_message: str, history: list = None,
                   telefono: str = "", client_id: int = None) -> str:
    """Contesta, y si hace falta usa la agenda. El modelo pide una herramienta,
    se la corremos, le devolvemos el resultado y sigue. Cuatro vueltas como
    máximo: alcanza para identificar, ver horas, apartar y agendar."""
    update_agent_status("recep", doing="Atendiendo cliente",
                        task=user_message[:80], status="active",
                        walk_x=0, walk_z=-0.9)

    # prompt_de_hoy() y no SYSTEM_PROMPT: las fechas tienen que ser las de hoy,
    # no las del día en que arrancó el proceso
    messages = [{"role": "system", "content": prompt_de_hoy()}]
    if history:
        messages.extend(history[-20:])
    messages.append({"role": "user", "content": user_message})

    def escalar(motivo, resumen):
        if client_id:
    ...
    for _ in range(4):
        try:
            ...
            try:
                ...
                logger.error(f"Error al llamar al modelo: {e2}")
                update_agent_status("recep", doing="Error", status="idle")
                return ("Lo siento, estoy teniendo problemas para conectarme en este "
                        "momento. Intenta de nuevo en unos segundos, o escribe "
                        "/asesor para hablar con el equipo de CJ Medical.")
        ...
        for tc in msg.tool_calls:
            update_agent_status("recep", doing=f"Agenda: {tc.function.name}",
                                status="active")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new_client:
        ...
        # La confirmación ya NO se adivina por palabras: «si no puedo»
        # quedaba como cita confirmada. La mueve confirmar_cita.
        if client["status"] == "pqrs" or client.get("ai_disabled"):
            ...
            # una pausa corta para que no se sienta un robot instantáneo
            await asyncio.sleep(1)
            reply = await ask_pepe(text, history,
    ...
    # El documento ya NO se adivina de cualquier número: un precio de 7
    # cifras terminaba guardado como cédula. Lo pide registrar_cliente.

    # ── Guardar en DB la respuesta del bot ──
    save_message(client["id"], "bot", reply, channel="telegram")

    # La cita ya NO se arma raspando el texto de Pepe ni se asigna la
    # especialista con una cadena de if: la crea agendar_cita contra la
    # agenda real, mirando disponibilidad.

    # Reprogramar y cancelar los maneja Pepe con sus herramientas.


    # ── Detectar queja/reclamo → pasar a "pqrs" ──
    pqrs_keywords = ["queja", "reclamo", "me quejo", "pqrs", "inconforme", "cobro indebido",
                      "mala atención", "mal servicio"]  # «problema», «error en» y
    # «devolución» salieron: eran demasiado comunes y el estado PQRS no tiene retorno
